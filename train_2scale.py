...
for train_size in train_sizes:
    ...
    f = open("log/1115/2scale_"+str(train_size),"w")
    sys.stdout = f
    for num_experiments_so_far in range(num_experiments):
        ...
        samples_x, samples_y, samples_y_aggregation, err_msg = lib_data.readCSVFile([dataset_filename2], inputs_name+['recommendation_pod0:rps','recommendation_pod1:rps','checkout_pod0:rps','checkout_pod1:rps'], "frontend:0.90")
        for sample_idx, sample_x, sample_y_aggregation in zip(range(len(samples_x)), samples_x, samples_y_aggregation):
            ...
            fluxion_input["recommendationservice:0.90"]["recommendationservice:0.90"][0]["recommendationservice:rps"] = sample_x[-4]
            fluxion_input["recommendationservice:0.90"]["recommendationservice:0.90"][1]["recommendationservice:rps"] = sample_x[-3]
