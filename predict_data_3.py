...
quantile = ["0.50", '0.90', '0.95', '0.85']
headers = ["service", "rps","avg", "0.50", '0.90', '0.95', '0.85']
perf = ["rps", "avg", "0.50", '0.90', '0.95', '0.85']
    ...
    for row in range(50):
        ...
        try:
            ...
            if r["service"] == "redis" or r["service"] == "total":
                continue
            elif r["service"][-4:-1]=="pod":
                # 有错位
                minidic[r["service"]+":rps"] = float(r["rps"])
                minidic[r["service"]+":0.50"] = float(r["avg"])
                minidic[r["service"]+":0.90"] = float(r["0.50"])
                minidic[r["service"]+":0.95"] = float(r["0.90"])
                minidic[r["service"]+":0.85"] = float(r["0.95"])
            else:
                for p in perf:
                    minidic[r["service"]+":"+p] = float(r[p])
        except:
            # print(route+"data"+str(i)+".csv")
            print(row)
            continue
    for i in range(size):
...
def standardize(csv, k):
    # load scalers
    dic = np.load("/home/yuqingxie/autosys/code/PlayGround/yuqingxie/100-std-scaler.npy", allow_pickle = True).item()
    name = k.split("_pod")
    if len(name) == 2:
        perf = name[1].split(":")[1]
        k = name[0]+":"+perf
...
def restruct(from_route, to_name, size=10000):
    ...
    with open(to_name) as csvfile:
        ...
        with open("dataset-3-standardized.csv", "w") as f:
...
if __name__ == "__main__":
    np.random.seed(0)
    random.seed(0)
    restruct("data-3.csv", "dataset-3.csv", 50000)