...
for pretrain_size in train_size:
    for num in range(num_experiments):
        f = open("log/1025/DNN_"+str(pretrain_size)+"_"+str(num),"w")
        sys.stdout = f
        path = "saved_model/DNN_"+str(pretrain_size)+"_"+str(num)
        name_list = np.load(path+".npy", allow_pickle = True)

        zoo = Model_Zoo()
        zoo.load("saved_model/DNN_"+str(pretrain_size)+"_"+str(num))
        fluxion = Fluxion(zoo)
        all_lrn_asgmts = {}
        ...
        for k in inputs_name:
            ...
            cnt += 1
        print(real_para)
        np.save("log/1025/res_DNN_"+str(pretrain_size)+"_"+str(num), real_para)
