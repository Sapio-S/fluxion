...
dataset_filename2 = "dataset-short-standardized.csv"
all_sample_x_names={}
...
inputs_name = expand_sample_x_name(target_service_name)
...
for pretrain_size in train_size:
    for num in range(num_experiments):
        # f = open("log/1028/GP_2checkout_"+str(pretrain_size)+"_"+str(num),"w")
        # sys.stdout = f
        path = "saved_model/GP_2checkout_"+str(pretrain_size)+"_"+str(num)
        name_list = np.load(path+".npy", allow_pickle = True)

        zoo = Model_Zoo()
        zoo.load("saved_model/GP_2checkout_"+str(pretrain_size)+"_"+str(num))
        fluxion = Fluxion(zoo)
        all_lrn_asgmts = {}
        ...
        for k in inputs_name:
            ...
            cnt += 1
        print(real_para)
        np.save("log/1028/GP_2checkout_"+str(pretrain_size)+"_"+str(num), real_para)
