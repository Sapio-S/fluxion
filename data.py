...
def store_input_valid(sub_map, i, train_size=500, test_size=84, valid_size=84):
    ...
    np.save("tmp_data_ms/"+str(i)+"_sample_x", a)
    np.save("tmp_data_ms/"+str(i)+"_sample_y", b)
    np.save("tmp_data_ms/names", c)
    np.save("tmp_data_ms/"+str(i)+"_perf_data", csvs)
    np.save("tmp_data_ms/"+str(i)+"_test_data", d)
    np.save("tmp_data_ms/"+str(i)+"_train_data", e)
    np.save("tmp_data_ms/"+str(i)+"_valid_data", f)
    np.save("tmp_data_ms/"+str(i)+"_csv_scale", csv_m)
    np.save("tmp_data_ms/"+str(i)+"_para_scale", para_m)

def generate_tmp_data_ms():
    print("generatring data. stored in tmp_data_ms/ folder.")
    for i in range(10):
        sub_map = np.arange(668)
        np.random.seed(i)
        np.random.shuffle(sub_map)
        store_input_valid(sub_map, i, 550, 118, 0) # no validation

if __name__ == "__main__":
    generate_tmp_data_ms()
