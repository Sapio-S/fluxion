...
def move_csvs():
    for i in range(50):
        shutil.move("res_collection/res0813/data"+str(i)+".csv", "res_merged_new/data"+str(i)+".csv")
    for i in range(100):
        shutil.move("res_collection/res0814/data"+str(i)+".csv", "res_merged_new/data"+str(i+50)+".csv")
    for i in range(500):
        shutil.move("res_collection/res0816/data"+str(i)+".csv", "res_merged_new/data"+str(i+150)+".csv")

def move_wrk_table():
    for i in range(50):
        shutil.move("wrk_collection/wrk_table0822/"+str(i), "wrk_rps_merged/"+str(i))
    for i in range(100):
        shutil.move("wrk_collection/wrk_table0827/"+str(i), "wrk_rps_merged/"+str(i+50))
    for i in range(500):
        shutil.move("wrk_collection/wrk_table0831/"+str(i), "wrk_rps_merged/"+str(i+150))
...
def rm_outlier(length):
    ...
    for i in range (length):
        if i in sub:
            ...
            for s in services:
                para[s].append(para_ori[s][i])
            shutil.copy("res_clean_rps/data"+str(i)+".csv", "res_rm_outlier_new/data"+str(cnt)+".csv")
            cnt += 1
            
    print(cnt)
    print(len(para["frontend"]))
    np.save("res_rm_outlier_new/param.npy", para)
