...
dataset_filename3 = "dataset-checkout2-standardized.csv"
samples_x0, samples_y0, samples_y_aggregation0, err_msg0 = lib_data.readCSVFile([dataset_filename3], inputs_name+['checkout_pod0:rps','checkout_pod1:rps'], target_service_name)
...
for train_size in train_sizes:
    ...
    for num_experiments_so_far in range(num_experiments):
        ...
        f = open("log/1111/prediction_"+str(train_size)+"_"+str(num_experiments_so_far),"w")
        sys.stdout = f
        ...
        #     cnt += 1
        # print(real_para)
        # np.save("log/1109/fluxion_2checkout_"+str(train_size)+"_"+str(num_experiments_so_far), real_para)
        # print(best * scaler["frontend:0.90:STD"] + scaler["frontend:0.90:AVG"])

        # numpy.save("saved_model/fluxion_2checkout_"+str(train_size)+"_"+str(num_experiments_so_far),zoo.get_models_name())
        # zoo.dump("saved_model/fluxion_2checkout_"+str(train_size)+"_"+str(num_experiments_so_far))
    ...
    #         # samples_x, samples_y, samples_y_aggregation, err_msg = lib_data.readCSVFile([dataset_filename2], inputs_name+["recommendation_pod0:rps", "recommendation_pod1:rps"], target_service_name)
    #         for sample_idx, sample_x, sample_y_aggregation in zip(range(len(samples_x)), samples_x, samples_y_aggregation):
    #             if sample_idx not in selected_testing_idxs:
    #                 continue
