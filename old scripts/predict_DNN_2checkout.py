...
for num_training_data in train_size:
    ...
    for num_experiments_so_far in range(10):
        f = open("log/1109/DNN_2checkout_"+str(num_training_data)+"_"+str(num_experiments_so_far),"w")
        sys.stdout = f
        ...
        for k in inputs_name:
            ...
            cnt += 1
        print(real_para)
        np.save("log/1109/DNN_2checkout_"+str(num_training_data)+"_"+str(num_experiments_so_far), real_para)
    ...
    #     print([round(statistics.mean(errs), 8) for errs in big_DNN_abs_errs])

    # # for collection
    # for errs in big_DNN_abs_errs:
