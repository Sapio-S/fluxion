...
train_size = [10, 25, 50, 100]
for num_training_data in train_size:
    f = open("log/1118/mix_"+str(num_training_data),"w")
    sys.stdout = f
    small_models_preds = []
    ...
    for num_experiments_so_far in range(num_experiments):
        ...
        selected_testing_idxs = random.sample(range(0, 190), k=num_testing_data)
        # train on 150
        selected_training_idxs1 = set(range(0, 190)) - set(selected_testing_idxs)
        selected_training_idxs1 = random.sample(selected_training_idxs1, k=100)
        # train on 100
        selected_training_idxs2 = random.sample(range(0, 800), k=600)
        # ========== Compute small models' errors ==========
        for sample_y_name in all_sample_x_names0.keys():
            sample_x_names = all_sample_x_names0[sample_y_name]
            # 100
            samples_x2, samples_y2, samples_y_aggregation2, err_msg = lib_data.readCSVFile([dataset_filename], sample_x_names, sample_y_name)
            # 150
            samples_x1, samples_y1, samples_y_aggregation1, err_msg = lib_data.readCSVFile([dataset_filename2], sample_x_names, sample_y_name)

            # STEP 1: Split dataset into training and testing
            training_samples_x = [samples_x1[idx] for idx in selected_training_idxs1]
            training_samples_x += [samples_x2[idx] for idx in selected_training_idxs2]
            training_samples_y_aggregation = [samples_y_aggregation1[idx] for idx in selected_training_idxs1]
            training_samples_y_aggregation += [samples_y_aggregation2[idx] for idx in selected_training_idxs2]
            testing_samples_x = [samples_x1[idx] for idx in selected_testing_idxs]
            testing_samples_y_aggregation = [samples_y_aggregation1[idx] for idx in selected_testing_idxs]
        ...
        training_samples_x = [samples_x[idx] for idx in selected_training_idxs1]
        training_samples_y_aggregation = [samples_y_aggregation[idx] for idx in selected_training_idxs1]
