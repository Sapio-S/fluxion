...
for pretrain_size in train_size:
    for num in range(num_experiments):
        # if num < 3:
        #     continue
        f = open("log/1031/fluxion_single"+str(pretrain_size)+"_"+str(num),"w")
        sys.stdout = f
        ...
        for k in inputs_name:
            ...
            cnt += 1
        print(real_para)
        np.save("log/1031/fluxion_single"+str(pretrain_size)+"_"+str(num), real_para)
