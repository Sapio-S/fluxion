...
for dataset in range(1,5):
    ...
    # f = open("log/1013/error_"+str(dataset),"w")
    # sys.stdout = f
    ...
    for sample_y_name in all_sample_x_names.keys():
        ...
        # # STEP3: calculate error distribution
        # samples_x, samples_y, samples_y_aggregation, err_msg = lib_data.readCSVFile([testfile], sample_x_names, sample_y_name)
        # selected_training_idxs = set(range(0, len(samples_x)))
        # training_samples_x = [samples_x[idx] for idx in selected_training_idxs]
        # training_samples_y_aggregation = [samples_y_aggregation[idx] for idx in selected_training_idxs]
        # all_lrn_asgmts[sample_y_name].set_err_dist(training_samples_x, training_samples_y_aggregation)
        # print(sample_y_name)
        # print("avg:", np.mean(training_samples_y_aggregation))
        # print("std:", np.std(training_samples_y_aggregation))
