...
"set", "recommendationservice", "cartservice", "checkoutservice","frontend"]#,"checkout_pod0","checkout_pod1"]
# finals = ["adservice", "currencyservice", "emailservice", 
# "paymentservice", "productcatalogservice","shippingservice", "get", 
...
def standardize(csv, k):
    # load scalers
    dic = np.load("/home/yuqingxie/autosys/code/PlayGround/yuqingxie/std_scaler_dataset_whole.npy", allow_pickle = True).item()
...
def restruct(from_route, to_name, size=10):
    ...
    with open(to_name) as csvfile:
        ...
        with open("dataset-test-standardized.csv", "w") as f:
...
if __name__ == "__main__":
    np.random.seed(0)
    random.seed(0)
    restruct("data-2checkout.csv", "dataset-test.csv", 1000)